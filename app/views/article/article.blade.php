...
@endif
    ...
    @elseif($isRepeat && $hideRepeat && !$isEditable)
        ...
        <a href="{{ preg_replace('/https/','http', URL::to($instanceName.'/archive/'.$article->originalPublication().'#article'.$article->id), 1) }}">
            <h1 id="articleTitle{{ $article->id }}" class="articleTitle{{ $isEditable ? ' editable' : '' }}">{{ $article->getTitle() }}<small>Click to read this article in its original publication</small></h1>
        </a>
