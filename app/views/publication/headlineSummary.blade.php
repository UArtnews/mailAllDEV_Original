...
@if($isEmail)
    ...
    @foreach($publication->articles as $article)
        @if($article->isPublished($publication->id) && $article->likeNew($publication->id) == 'N' )
            {{ str_replace('**HEADLINE**', '<a href="'.preg_replace('/https/','http', URL::to($instanceName.'/archive/'.$publication->id.'#repeate-container-header'), 1).'">Repeated Items</a><br/>', isset($tweakables['publication-headline-summary-style']) ? $tweakables['publication-headline-summary-style'] : $default_tweakables['publication-headline-summary-style']) }}
            <?break;?>
        @endif
    @endforeach
