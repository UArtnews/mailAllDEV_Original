@if($isEmail)
    ...
    @foreach($publication->articles as $article)
        {{ str_replace('**HEADLINE**', '<a href="#articleTitle'.$article->id.'">'.strip_tags($article->title).'</a><br/>', isset($tweakables['publication-headline-summary-style']) ? $tweakables['publication-headline-summary-style'] : $default_tweakables['publication-headline-summary-style']) }}
    @endforeach
